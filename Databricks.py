...
# Sort by education group
# Get top bachelor_rate
result = spark.sql(
  """
    SELECT 
      education,
...
# MAGIC
# MAGIC For simplicity we are going to use the plotting functionality built into pandas (you could make this as pretty as you want). 
...
# Step 2: Create instance of GBTClassifier model
# Should labelCol be ='indexed', 'label', or 'features'?
# Should there be an argument leafCol='leafID'?
gb = GBTClassifier(labelCol="label", maxIter=5, maxDepth=2)

# documentation
# gbt = GBTClassifier(maxIter=5, maxDepth=2, labelCol="indexed", seed=42, leafCol="leafId")

# example for LogisticRegression model
# lr = LogisticRegression(labelCol="label", featuresCol="features", maxIter=10)


# Step 3: Set threshold? On the documentation, it only applies to model?
# set threshold for the probability above which to predict a 1
gb.setThresholds([0.5, training_data_positive_rate])

# Step 4: Train model with training data
gbModel = gb.fit(trainingData)

print ('done')
# Step 5: Get training summary for eval metrics and other params
# gbTrainingSummary = gbModel.summary

# Step 6: Find best model threshold if you would like to use that instead of the empirical positive rate
...
# MAGIC %md
# MAGIC
# MAGIC ####Logistic Regression - Param Grid
...
#         .schema[gbModel.summary.featuresCol] # not sure what to replace instead of '.summary'
#         .metadata["ml_attr"]["attrs"].values())))


# [(name, gbModel.summary.pValues[idx]) for idx, name in attrs]

# [(name, gbModel.coefficients[idx]) for idx, name in attrs]
...
gbWeights = gbCvModel.bestModel.featureImportances
gbWeights = [(float(w),) for w in gbWeights]  # convert numpy type to float, and to tuple
gbFeatureImportance = sqlContext.createDataFrame(gbWeights, ["Feature Weight"])
display(gbFeatureImportance)
...
rf = RandomForestClassifier(numTrees=3, maxDepth=2, labelCol="label")
...
rfWeights = rfCvModel.bestModel.featureImportances
rfWeights = [(float(w),) for w in rfWeights]  # convert numpy type to float, and to tuple
rfFeatureImportance = sqlContext.createDataFrame(rfWeights, ["Feature Weight"])
